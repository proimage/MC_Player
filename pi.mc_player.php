...
class Mc_player
{
	...
	// Validate parameter values
		...
		elseif ($SESS->cache['mc']['player']['id'] != '')
		{
			$container_id = 'player_' . str_replace('.','-',$SESS->cache['mc']['player']['id']); // Use ID prepared downstream
		}
		...
		if ( ctype_digit($TMPL->fetch_param('width')) ) {
			$native_width = $TMPL->fetch_param('width');
		} else {
			$native_width = 300; // Default width of <video> element?
			$TMPL->log_item("WARNING in MC Player plugin: Specified 'width' is not an integer; defaulting to 300");
		}
		...
		if ( ctype_digit($TMPL->fetch_param('height')) ) {
			$native_height = $TMPL->fetch_param('height');
			...
			$TMPL->log_item("WARNING in MC Player plugin: Specified 'height' is not an integer; defaulting to 150");
		}
		
		if ( ctype_digit($TMPL->fetch_param('fit_in_width')) ) {
			$fit_in_width = $TMPL->fetch_param('fit_in_width');
		...
		// Other
		$playerpath = ($TMPL->fetch_param('playerpath')) ? $TMPL->fetch_param('playerpath') : '';
		$bgcolor = ($TMPL->fetch_param('bgcolor')) ? $TMPL->fetch_param('bgcolor') : '';
		$image = ($TMPL->fetch_param('image')) ? $TMPL->fetch_param('image') : '';
		$link = ($TMPL->fetch_param('link')) ? $TMPL->fetch_param('link') : '';
		if ($TMPL->fetch_param('autostart'))
		{
			$autostart = $TMPL->fetch_param('autostart');
		}
		elseif ($TMPL->fetch_param('autoplay')) // because I kept forgetting which 'auto-' to use :p
		{
			$autostart =  $TMPL->fetch_param('autoplay');
		}
		$bufferlength = ($TMPL->fetch_param('bufferlength')) ? $TMPL->fetch_param('bufferlength') : '';
		$displayclick = ($TMPL->fetch_param('displayclick')) ? $TMPL->fetch_param('displayclick') : '';
		$fullscreen = ($TMPL->fetch_param('fullscreen')) ? $TMPL->fetch_param('fullscreen') : '';
		$mute = ($TMPL->fetch_param('mute')) ? $TMPL->fetch_param('mute') : '';
		$volume = ($TMPL->fetch_param('volume')) ? $TMPL->fetch_param('volume') : '80';
		$showmute = ($TMPL->fetch_param('showmute')) ? $TMPL->fetch_param('showmute') : '';
		
		// Controlbar
		if ($TMPL->fetch_param('controlbar'))
		{
			$controlbar = $TMPL->fetch_param('controlbar');
		...
		switch ($TMPL->fetch_param('provider'))
			{
				...
				case 'rtmp':
				case 'youtube':
					$provider = $TMPL->fetch_param('provider');
				case false:
					break;
				default:
		...
		// Playlist stuff
		$shuffle = ($TMPL->fetch_param('shuffle')) ? $TMPL->fetch_param('shuffle') : '';
		
		switch ($TMPL->fetch_param('repeat')) // input value verification
		{
			case "none":
			case "list":
			case "always":
			case "single":
				$repeat = $TMPL->fetch_param('repeat');
				break;
			case false:
				break;
			default:
		...
		{
			$specified_pl_position = $SESS->cache['mc']['player']['playlist_position'];
				unset($SESS->cache['mc']['player']['playlist_position']);
		}
		elseif ($TMPL->fetch_param('playlist_position') !== false)
		{
			$specified_pl_position = $TMPL->fetch_param('playlist_position');
		}
		// take into account if a playlist size was provided without a position (intent is to show playlist)
		elseif ($TMPL->fetch_param('playlist_size') !== false)
		{
		...
		{
			$pl_size = $SESS->cache['mc']['player']['playlist_size'];
...
/*
		if ($container_tag != 'video') // Flash player gets a lot more properties
		{
*/
...
*/
	...
	function playlist()
	{
		...
		$SESS->cache['mc']['player']['playlist_position'] = ($TMPL->fetch_param('position')) ? $TMPL->fetch_param('position') : '';
		$SESS->cache['mc']['player']['playlist_size'] = ($TMPL->fetch_param('size')) ? $TMPL->fetch_param('size') : '';
		...
		{
			$TMPL->log_item("ERROR in MC Player plugin: 'playlist' container specified, but no 'item' items were found");
			return $TMPL->no_results();
		}
	...
	function item()
	{
		...
		// Assignment
		$file = ($TMPL->fetch_param('file')) ? $TMPL->fetch_param('file') : '';
		$image = ($TMPL->fetch_param('image')) ? $TMPL->fetch_param('image') : '';
		$duration = ($TMPL->fetch_param('duration')) ? $TMPL->fetch_param('duration') : NULL;
		$start = ($TMPL->fetch_param('start')) ? $TMPL->fetch_param('start') : NULL;
		$title = ($TMPL->fetch_param('title')) ? $TMPL->fetch_param('title') : '';
		$description = ($TMPL->fetch_param('description')) ? $TMPL->fetch_param('description') : '';
		$streamer = ($TMPL->fetch_param('streamer')) ? $TMPL->fetch_param('streamer') : '';
		$provider = ($TMPL->fetch_param('provider')) ? $TMPL->fetch_param('provider') : '';
		$levels = ($SESS->cache['mc']['player']['levels']) ? $SESS->cache['mc']['player']['levels'] : '';
		unset($SESS->cache['mc']['player']['levels']);
	...
	// Validation & Formatting
		...
		if ($levels)
		{
			...
			if ($file) $TMPL->log_item("NOTICE in MC Player plugin: 'file' specified for playlist item when 'levels' already specified; ignoring 'file'");
		}
		elseif ($file)
		{
			$SESS->cache['mc']['player']['id'] .= "_" . basename(html_entity_decode($file)); // used for auto-naming container ID
		...
		$SESS->cache['mc']['player']['items'] .= $item;
	...
	function level()
	{
		...
		$width = ($TMPL->fetch_param('width')) ? $TMPL->fetch_param('width') : '';
		$file = ($TMPL->fetch_param('file')) ? $TMPL->fetch_param('file') : '';
	...
	function mode()
	{
		...
		switch ($type = $TMPL->fetch_param('type'))
		{
			case 'flash':
				if ($src = $TMPL->fetch_param('src'))
				{
		...
		{
			$mode_params = PHP_EOL . "config: {" . trim($SESS->cache['mc']['player']['mode']['config'], ",") . PHP_EOL . "}";
		...
		// store result in session for use upstream
		$SESS->cache['mc']['player']['mode_list'] .= $mode;
